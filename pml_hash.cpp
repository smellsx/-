...
size_t PMLHash:: determine_location(size_t* arr)
{
	for(size_t i=0; i<BITMAP ; i++){	
        ...
        if(arr[i] < SIZE_MAX){	//找到第一个存在0的bitmap

            size_t bitMask = 1;

            bitMask <<= 63;		//bitMask左移63位

            for(size_t j=0; j<64; j++){

                if( (bitMask & arr[i]) == 0){	//若结果为0 说明找到了一个0位置
...
void PMLHash:: set_zero(size_t num,size_t* arr)
{
    ...
    for(size_t index = 0; index < j ; index++){		//定位
